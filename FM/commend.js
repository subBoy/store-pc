...
var wheelStatus1 = true;
var lastEditRange1;
function renderEmoji1 (arr) {
	var emDom = '';
	for (var i = 0; i < arr.length; i++) {
		emDom += '<span class="emoji-img-item1" emoji="' + arr[i] + '">'
								+'<i class="emoji-img-ctr in-block ke ke-' + arr[i] + '"></i>'
							+'</span>';
	}
	$('#emojiImgList1').html(emDom);
	var navDom = '';
	var navNum = arr.length / 50;
	navNum = Math.ceil(navNum);
	for (var j = 0; j < navNum; j++) {
		...
		navDom += '<span class="in-block emimg-nav1 ' + seledClass + '"></span>'
	}
	$('.emoji-img-nav1').html(navDom);

	var nIdx = 0;
	$('#emojiImgList1').bind('mousewheel', function(event, delta) {
		if (!wheelStatus1) {
			return false;
		}
		wheelStatus1 = false;
	...
	$('.emimg-nav1').click(function () {
		nIdx = $(this).index();
		emojiAn1(nIdx, 130);
	})
	$('.emoji-img-item1').click(function (e) {
		e.stopPropagation();
		var selection = getSelection()
      ...
      selection.addRange(lastEditRange)
		...
		var zLen = jsEmojiLen1();
		var emoji = $(this).attr('emoji');
		var img = document.createElement("IMG");
		img.setAttribute("class", "ke in-block ke-" + emoji);
		img.setAttribute("src", "../FM/images/opacity.png");
		var edit = document.getElementById('emoji-input-wrapper1');
		edit.focus();
		var s = window.getSelection().getRangeAt(0);
    s.insertNode(img);
    if (zLen > 120) {
...
function emojiAn1 (idx, xh) {
	$('.emimg-nav1').eq(idx).addClass('seled').siblings('.emimg-nav1').removeClass('seled');
	$('#emojiImgList1').animate({
		top: '-' + idx * xh + 'px'
	}, 500, function () {
		wheelStatus1 = true;
	})
...
function jsEmojiLen1 () {
	var emHtml = $('#emoji-input-wrapper1').html();
	$('#emoji-len-num-wrapper1').html(emHtml);
	var imgLen = $('#emoji-input-wrapper1').find('img').length;
	$('#emoji-len-num-wrapper1').find('img').remove();
	var txtBox = $('#emoji-len-num-wrapper1').html();
	var txtLen = txtBox.length;
	return imgLen * 3 + txtLen;
}

function renderTextNode1 (txt) {
	var emojiText = document.createTextNode(txt);
	var edit = document.getElementById('emoji-input-wrapper1');
	edit.focus();
	var selection = getSelection()
  ...
  var zLen = jsEmojiLen1();
  if (zLen > 120) {
		$('.emoji-submit-err1').fadeIn();
	} else {
		$('.emoji-submit-err1').fadeOut();
	}
  $('.emoji-txt-len1').html(zLen + '/120');
  var e = document.createRange();
  e.setStartAfter(emojiText);
  e.setEndAfter(emojiText);
  if (document.getSelection) {
...
function isReport1 () {
	$('.slcl-btn-11, .essay-btn-51').on('click', function () {
		$('.groove-win-wrapper1').fadeIn();
	})
	$('.groove-win-close-btn1, .groove-win-sumbit-btn1').on('click', function () {
		$('.groove-win-wrapper1').fadeOut();
		$('.sel-geim-checkbox1').attr('checked', false);
		$('.sel-geim-checkbox1').parent().removeClass('seled');
	})
	$('.slcl-btn-31').click(function () {
		replayId = $(this).parent().parent().find('.slcl-item-desc-txt1').html();
		var userName = $(this).attr('userName');
		var cmTxt = '回复' + userName + '：';
		$('#emoji-input-wrapper1').html('');
		renderTextNode1(cmTxt);
	})
	$('.slcl-btn-21').click(function () {
		var cdLen = $(this).attr('cdLen');
		if ($(this).hasClass('seled')) {
			cdLen = cdLen - 1;
			$(this).removeClass('seled')
...
$(function () {
	...
	isReport1();
	renderEmoji1(imgClass);
	$('#emoji-input-wrapper').on('click', function () {
		var selection = getSelection()
		lastEditRange = selection.getRangeAt(0);
	})
	...
	$('.emoji-submit-btn').click(function () {
		...
		var isLook = '';
		if (isl) {
			isLook = '<p class="slcl-item-desc-model">'
								+'<span class="sidm-txt in-block">在有屁就放#评论了 :</span>'
								+'<span class="sidm-tle in-block">无题</span>'
							+'</p>'
		}
	...
	$('#emoji-input-wrapper1').on('click', function () {
		var selection = getSelection()
		lastEditRange = selection.getRangeAt(0);
	})
	$('#emoji-input-wrapper1').on('input', function () {
		var zLen = jsEmojiLen1();
		if (zLen > 120) {
			$('.emoji-submit-err1').html('评论字数超出最大值范围！');
			$('.emoji-submit-err1').fadeIn();
		} else {
			$('.emoji-submit-err1').fadeOut();
		}
	...
	$('.emoji-btn-11').click(function (e) {
		e.stopPropagation();
		var edit = document.getElementById('emoji-input-wrapper1');
		edit.blur();
		$('.emoji-img-wrapper1').slideDown();
	})
	$('.emoji-btn-21').click(function () {
		renderTextNode1('@');
	})
	$('.emoji-btn-31').click(function () {
		var emVal = $('#emoji-input-wrapper1').html();
		var emSta = emVal.indexOf('#输入想说的话题#');
		if (emSta !== -1) {
			return;
		}
		renderTextNode1('#输入想说的话题#');
	})
	$('.emoji-submit-btn1').click(function () {
		var zLen = jsEmojiLen1();
		if (zLen > 120) {
			return;
		}
		if (zLen < 1) {
			$('.emoji-submit-err1').html('写点东西吧，内容不能为空哦！');
			$('.emoji-submit-err1').fadeIn();
			return;
		}
		var cdCt = $('#emoji-input-wrapper1').html();
		$('#lg-dom1').html(replayId);
		var cssTxt = '回复' + $('#lg-dom1').find('.slcl-item-user-name').html();
		var cssTxt2 = '@' + $('#lg-dom1').find('.slcl-item-user-name').html();
		var xxHtml = '';
		$('#lg-dom1').find('.slcl-item-user-name').html(cssTxt2);
		if (replayId && cdCt.indexOf(cssTxt) !== -1) {
			cdCt = cdCt.split(cssTxt)[1];
			xxHtml = '<div class="replay-slcl-item">' + $('#lg-dom1').html() + '</div>';
		}
		var reHtml = '<li class="slcl-item">'
						+'<div class="slcl-item-user-avatar">'
							+'<img src="../FM/images/song-poster.jpg" width="100%" height="100%">'
						+'</div>'
						+'<p class="slcl-item-desc-txt1">'
							+'<span class="slcl-item-user-name">aabbcc：</span>' + cdCt
						+'</p>'
						+'<div class="replay-slcl-item-wrapper">' + xxHtml + '</div>'
						+'<p class="slcl-item-time-1">'
							+'<span class="in-block sit-1">2018年11月21日 12 : 23</span>'
							+'<span class="in-block sit-2">你对他/她说了悄悄话!</span>'
						+'</p>'
						+'<div class="slcl-item-opr-btn esp">'
							+'<span class="in-block slcl-btn set-animation slcl-btn-41">删除</span>'
							+'<span class="in-block slcl-btn-hr"></span>'
							+'<a href="#gmBtn1" class="in-block slcl-btn set-animation slcl-btn-31" userName="aabbcc">回复</a>'
						+'</div>'
					+'</li>'
		$('#latest-commend-list1').prepend(reHtml);
		$('#emoji-input-wrapper1').html('');
		$('.emoji-txt-len1').html('0/120');
		$('.slcl-btn-11, .slcl-btn-21, .slcl-btn-31, .slcl-btn-41').unbind();
		isReport1();
		$('.emoji-submit-err1').html('评论成功！');
		$('.emoji-submit-err1').fadeIn();
		setTimeout(function () {
			$('.emoji-submit-err1').fadeOut();
		}, 2000);
